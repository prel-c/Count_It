...
def test(image, template):   
    ...
    def template_search(magnitude_im, magnitude_te, threshold=0.6):
        """
        ...
        all_rectangles=[]
        h, w = template.shape
        result=cv2.matchTemplate(magnitude_im, magnitude_te, cv2.TM_CCOEFF_NORMED)    # Шаблонный поиск
    ...
    def NMS(rectangles, iou_threshold=0.3):
        """
        Функция удаления прямоугольников, дублирующих существующий (подавление немаксимумов)
    ...
    h_te, w_te=template.shape
    for scale in scales:

        h_te_new, w_te_new=h_te*scale, w_te*scale
        template_new=cv2.resize(template, (int(h_te_new), int(w_te_new)))
        template_blur=cv2.GaussianBlur(template_new, (5, 5), 41)                            # Размытие по Гауссу для шаблона
        magnitude_te=Sobel(template_blur)                                             # Применение ядра Собеля к шаблону

        rectangles=template_search(magnitude_im, magnitude_te, threshold=0.4)
        rectangles=NMS(rectangles, iou_threshold=0.3)
        
        lbp_template=LBP(template_blur)                                                 # Матрица LBP для шаблона
        hist_template=LBP_hist(lbp_template)                                            # Гистограмма LBP для шаблона
    ...
    return len(valid_rectangles)                                                    # Возвращаем длинну массива удовлетворяющих областей