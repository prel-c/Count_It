...
def cheking(image, rectangle, ghist1):
    ...
    if chi_square(ghist, ghist1, epsilon=1e-10) < 0.3:
...
def merge_duplicate_rectangles(rectangles, threshold=0.3):
    ...
    while len(sorted_idx) > 0:
        # Берем самый большой прямоугольник
...
for i in range (0, len(template)*2//3):
    new_size = (len(template)-i, len(template[0])-i)  # (ширина, высота)
    Borders_te = cv2.resize(Borders_te, new_size, interpolation=cv2.INTER_AREA)
    template_copy = cv2.resize(template_copy, new_size, interpolation=cv2.INTER_AREA)
    rectangles, count_template_search=template_search(Borders_im, Borders_te)
