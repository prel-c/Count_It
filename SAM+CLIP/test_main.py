...
def test(device, data_path, names_images, row_colors):
    ...
    for im_id in pbar:
        ...
        #image = Image.open(f"{im_dir}, {im_id}") #если нужно считать изображение через PIL

        image = cv2.imread(os.path.join(im_dir, im_id)) #если нужно считать изображение через openCV
        ...
        ref_point=[(examples[0][1]+examples[0][3])/2, (examples[0][0]+examples[0][2])/2]

        #вызов функций для обратки и подсчёта
        ref_feat, _, _, _ = get_reference(
            image,
            ref_point
        )

        # search
        output, _, _, _ = find_similar_objects(
            image,
            ref_feat
        )
        ...
        set_description=(f'{base_color}{im_id:<10}'
...
if __name__ == "__main__":
    ...
    names_images = 'my' #выбор по какой выборке из датасета делать тестирование
